<template>
  <div class="detail app-init gouwuche-page" id="transition">
    <header-back title="资讯详情"></header-back>
...
export default {
  ...
  methods: {
    btn(list) {
      ...
      let idExist = this.$store.state.article.find(data => {
  ...
  created() {
    ...
    axios.get("/static/server/discover.json").then(function(res) {
